          ...
          {steps.map((step, index) => (
            ...
            >
              ...
              <div className="card p-8 h-full">
                {/* Step Number Badge */}
                <div className="absolute -top-3 left-8 flex h-7 items-center justify-center rounded-full bg-[var(--color-primary)] px-3 text-xs font-semibold text-white">
                  {step.number}
                </div>

                {/* Icon */}
                <div className="mt-2 flex h-14 w-14 items-center justify-center rounded-xl bg-[var(--color-primary-subtle)] text-[var(--color-primary)] transition-all duration-200 group-hover:bg-[var(--color-primary)] group-hover:text-white">
                  <step.icon className="h-7 w-7" />
                </div>

                {/* Content */}
                <h3 className="mt-6 font-serif text-xl text-[var(--color-text-primary)]">
                  {step.title}
                </h3>
              ...
              {index < steps.length - 1 && (
                <div className="absolute right-0 top-1/2 hidden h-px w-8 -translate-y-1/2 translate-x-full bg-[var(--color-border)] md:block" />
              )}
            </div>
          ))}
        </div>
