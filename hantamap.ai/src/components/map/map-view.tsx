...
export function MapView({
  ...
  useEffect(() => {
    ...
    reports.forEach(report => {
      ...
      const icon = L.divIcon({
        ...
        html: `<div style="position:relative;width:${size}px;height:${size}px;">
          <div class="signal-pulse" style="position:absolute;inset:0;border-radius:50%;background:rgba(255,77,87,0.2);"></div>
          ${isApprox ? `<div style="position:absolute;inset:2px;border-radius:50%;border:1.5px dashed rgba(255,77,87,0.3);"></div>` : ''}
          <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:10px;height:10px;border-radius:50%;background:#ff4d57;border:2px solid rgba(255,255,255,0.9);box-shadow:0 0 12px rgba(255,77,87,0.5);"></div>
        </div>`,
      })
      ...
      const popup = `<div style="min-width:200px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px;">
          <div style="background:rgba(255,255,255,0.04);border-radius:8px;padding:8px;">
            ...
            <div style="font-size:16px;font-weight:700;color:#f2f7f8;">${cases}</div>
          </div>
          <div style="background:rgba(255,255,255,0.04);border-radius:8px;padding:8px;">
            <div style="font-size:9px;color:#5a7078;text-transform:uppercase;letter-spacing:0.3px;">Deaths</div>
            <div style="font-size:16px;font-weight:700;color:#f2f7f8;">${deaths}</div>
          </div>
        </div>
        ${isApprox ? '<div style="color:#ffb240;font-size:10px;margin-bottom:4px;">Location is approximate</div>' : ''}
        ${report.editor_note ? `<div style="color:#5a7078;font-size:10px;font-style:italic;border-top:1px solid rgba(255,255,255,0.06);padding-top:6px;margin-top:4px;">${report.editor_note.slice(0, 150)}${report.editor_note.length > 150 ? '...' : ''}</div>` : ''}
      </div>`
      ...
      mediaItems.forEach(item => {
        ...
        const icon = L.divIcon({
          ...
          html: `<div style="position:relative;width:20px;height:20px;">
            <div class="media-pulse" style="position:absolute;inset:0;border-radius:50%;background:rgba(255,178,64,0.15);"></div>
            <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);width:8px;height:8px;border-radius:50%;background:#ffb240;border:2px dotted rgba(255,255,255,0.6);box-shadow:0 0 8px rgba(255,178,64,0.4);"></div>
          </div>`,
        })
        ...
        const popup = `<div style="min-width:200px;">
          ...
          <div style="color:#9fb0b7;font-size:11px;margin-bottom:6px;">${item.publisher || item.original_publisher || 'Unknown'} · ${pubDate}</div>
          ...
          <div style="color:#5a7078;font-size:10px;margin-bottom:8px;">Not verified by health authorities. Case counts: Unknown.</div>
    ...
    // Auto-fit to markers if any exist
