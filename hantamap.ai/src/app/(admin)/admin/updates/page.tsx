...
export const metadata: Metadata = { title: 'Admin: Updates' }

export default async function AdminUpdatesPage() {
  const supabase = await createClient()
  const { data: updates } = await supabase
    .from('updates')
    .select('*, outbreak:outbreaks(name)')
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/admin/updates/new" className="bg-slate-900 text-white text-sm font-medium px-4 py-2 rounded hover:bg-slate-800">
          Create update
        </Link>
      </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {updates.map((u: any) => (
                <tr key={u.id} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-500">{u.outbreak?.name || 'None'}</td>
                  <td className="px-4 py-3"><VerificationBadge status={u.verification_status} /></td>
                  <td className="px-4 py-3">
                    <span className={`text-xs font-medium ${u.published ? 'text-emerald-600' : 'text-slate-400'}`}>
                      {u.published ? 'Published' : 'Draft'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <Link href={`/admin/updates/${u.id}`} className="text-xs text-slate-500 hover:text-slate-900">Edit</Link>
                  </td>
                </tr>
              ))}
            </tbody>
